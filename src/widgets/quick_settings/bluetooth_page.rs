...
impl BluetoothPage {
    pub fn new(
        ...
        let title = gtk4::Label::builder().label("Bluetooth").halign(gtk4::Align::Start).css_classes(vec!["qs-subpage-title".to_string()]).build();
        ...
        let ctx_c = ctx.clone();
        let mut bt_rx = ctx.bluetooth_rx.clone();
        
        // State-Tracking
        let last_device_ids = Rc::new(RefCell::new(Vec::<String>::new()));

        gtk4::glib::MainContext::default().spawn_local(async move {
            Self::update_ui(&bt_rx.borrow(), &bt_tile, &list_c, &ctx_c, &last_device_ids);

            while bt_rx.changed().await.is_ok() {
                Self::update_ui(&bt_rx.borrow(), &bt_tile, &list_c, &ctx_c, &last_device_ids);
            }
        });
    ...
    fn update_ui(
        ...
        let current_ids: Vec<String> = data.devices.iter().map(|d| format!("{}-{}", d.path, d.is_connected)).collect();
        {
        ...
        *last_device_ids.borrow_mut() = current_ids;
        ...
        for dev in &data.devices {
            let sublabel = if dev.is_connected { Some("Connected") } else if dev.is_paired { Some("Paired") } else { None };
