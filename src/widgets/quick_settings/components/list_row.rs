...
impl QsListRow {
    pub fn new(label: &str, icon: &str, active: bool, sublabel: Option<&str>) -> Self {
        ...
        let button = gtk4::Button::builder().css_classes(vec!["qs-list-row".to_string()]).focusable(false).build();
        ...
        label_box.append(&gtk4::Label::builder().label(label).halign(gtk4::Align::Start).build());
        
        if let Some(sub) = sublabel {
            label_box.append(&gtk4::Label::builder().label(sub).halign(gtk4::Align::Start).css_classes(vec!["qs-list-sublabel".to_string()]).build());
        }
