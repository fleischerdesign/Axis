...
impl LauncherProvider for FileSearchProvider {
    fn id(&self) -> &str {
        "files"
    }
    ...
    fn search<'a>(
        ...
        Box::pin(async move {
            ...
            // Locate + Filter in Background-Thread — blockiert nicht den Main Thread
            ...
            // Empfange Ergebnisse (async, nicht blockierend)
            match rx.recv().await {
                Ok(results) => results,
                Err(_) => vec![],
            }
...
/// Blocking search logic — läuft im Background-Thread
impl FileSearchProvider {
