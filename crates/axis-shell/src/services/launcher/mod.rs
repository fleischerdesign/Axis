...
use axis_core::{ServiceStore, Store};
use log::{error, info};
use std::rc::Rc;
use std::sync::Arc;
...
impl Service for LauncherService {
    ...
    fn spawn() -> (ServiceStore<Self::Data>, Sender<Self::Cmd>) {
        ...
        let providers: Vec<Arc<dyn LauncherProvider>> = vec![
            Arc::new(providers::apps::AppProvider::default()),
            Arc::new(providers::files::FileSearchProvider),
            Arc::new(providers::web::WebSearchProvider::default()),
        ];

        let providers_ref = Rc::new(RefCell::new(providers));
                ...
                *pending_cmd_clone.borrow_mut() = Some(cmd);
                ...
                glib::idle_add_local(move || {
                    if let Some(cmd) = pending.borrow_mut().take() {
                        handle_cmd(cmd, &providers, &cancel, &store);
                    }
    ...
    match cmd {
        LauncherCmd::SelectNext => {
            data_store.update(|d| {
                ...
                let next = d
                    .selected_index
                    .map_or(0, |i| (i + 1).min(d.results.len() - 1));
                    ...
                    match &item.action {
                        LauncherAction::Exec(program) => {
                            ...
                            if let Err(e) = Command::new("sh")
                                ...
                                .spawn()
                            {
                                error!("[launcher] Failed to execute: {program} ({e})");
                            }
                        }
                        LauncherAction::OpenUrl(url) => {
                            ...
                            if let Err(e) = Command::new("xdg-open")
                                ...
                                .spawn()
                            {
                                error!("[launcher] Failed to open URL: {url} ({e})");
                            }
                        }
        ...
        LauncherCmd::Search(query) => {
            ...
            *search_cancel.borrow_mut() = cancel.clone();
            ...
            let active_providers: Vec<Arc<dyn LauncherProvider>> =
                providers_ref.borrow().clone();
            ...
            glib::spawn_future_local(async move {
                while let Ok(mut results) = result_rx.recv().await {
                    ...
                    data_store_clone.update(move |d| {
                        ...
                        d.is_searching = true;
                        d.selected_index =
                            if d.results.is_empty() { None } else { Some(0) };
