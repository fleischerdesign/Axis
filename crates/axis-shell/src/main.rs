...
fn build_ui(app: &libadwaita::Application, start_locked: bool, wallpaper_path: Option<String>) {
                ...
                if last_notified.borrow().as_deref() != Some(peer_id) {
                    *last_notified.borrow_mut() = Some(peer_id.clone());
                    ...
                    on_action.insert(
                        ...
                        std::sync::Arc::new({
                            let tx = tx_c.clone();
                            move || { let _ = tx.try_send(axis_core::services::continuity::ContinuityCmd::ConfirmPin); }
                        })
                    );
                    on_action.insert(
                        "reject".to_string(),
                        std::sync::Arc::new({
                            let tx = tx_c.clone();
                            move || { let _ = tx.try_send(axis_core::services::continuity::ContinuityCmd::RejectPin); }
                        })
                    );
    ...
    // --- D-BUS HOST SPAWNEN (IPC + Settings) ---
