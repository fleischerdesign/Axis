...
impl View<TrayStatus> for TrayWidget {
    fn render(&self, status: &TrayStatus) {
        ...
        for item in items {
            let exists = self.icons.borrow().contains_key(&item.bus_name);
            if exists {
                let img = self.icons.borrow().get(&item.bus_name).cloned();
                if let Some(img) = img {
                ...
                self.inner_box.append(&img);
                self.icons.borrow_mut().insert(item.bus_name.clone(), img);
            }
        }

        self.island.container.set_visible(!items.is_empty());
    }
}
