...
use axis_domain::ports::tasks::{TaskError, TaskProvider};
use log::debug;
...
impl TaskProvider for GoogleTasksProvider {
    async fn get_lists(&self) -> Result<Vec<TaskList>, TaskError> {
        ...
        let resp = self
            .http_client
            .get(format!("{}/users/@me/lists", GOOGLE_TASKS_API_URL))
            .bearer_auth(&token)
            .send()
            .await
            .map_err(|e| TaskError::ProviderError(e.to_string()))?;

        let list_resp: GoogleTaskListResponse = resp
            .json()
            .await
            .map_err(|e| TaskError::ProviderError(e.to_string()))?;

        let lists: Vec<TaskList> = list_resp
            .items
            .unwrap_or_default()
            .into_iter()
            .map(|l| TaskList {
    ...
    async fn get_tasks(&self, list_id: &str) -> Result<Vec<Task>, TaskError> {
        ...
        let resp = self
            .http_client
            .get(format!("{}/lists/{}/tasks", GOOGLE_TASKS_API_URL, list_id))
            .bearer_auth(&token)
            .send()
            .await
            .map_err(|e| TaskError::ProviderError(e.to_string()))?;

        let task_resp: GoogleTaskResponse = resp
            .json()
            .await
            .map_err(|e| TaskError::ProviderError(e.to_string()))?;

        let tasks: Vec<Task> = task_resp
            .items
            .unwrap_or_default()
            .into_iter()
            .map(|t| Task {
        ...
        debug!(
            "[google-tasks] Fetched {} tasks from list {}",
            tasks.len(),
            list_id
        );
        Ok(tasks)
    ...
    async fn toggle_task(&self, list_id: &str, task_id: &str, done: bool) -> Result<(), TaskError> {
        ...
        let resp = self
            ...
            .patch(format!(
                "{}/lists/{}/tasks/{}",
                GOOGLE_TASKS_API_URL, list_id, task_id
            ))
            .bearer_auth(&token)
        ...
        debug!(
            "[google-tasks] Toggled task {} in list {} to done={}",
            task_id, list_id, done
        );
        Ok(())
    }
    ...
    async fn delete_task(&self, list_id: &str, task_id: &str) -> Result<(), TaskError> {
        ...
        debug!(
            "[google-tasks] Deleted task {} from list {}",
            task_id, list_id
        );
        Ok(())
    }
    ...
    async fn create_task(&self, list_id: &str, title: &str) -> Result<Task, TaskError> {
        ...
        let resp = self
            .http_client
            .post(format!("{}/lists/{}/tasks", GOOGLE_TASKS_API_URL, list_id))
            .bearer_auth(&token)
        ...
        let google_task: GoogleTaskItem = resp
            .json()
            .await
            .map_err(|e| TaskError::ProviderError(e.to_string()))?;

        debug!(
            "[google-tasks] Created task '{}' in list {}",
            title, list_id
        );
    ...
    async fn get_auth_status(&self) -> Result<axis_domain::models::cloud::AuthStatus, TaskError> {
        match self.auth_provider.is_authenticated().await {
            Ok(true) => Ok(axis_domain::models::cloud::AuthStatus::Authenticated),
            Ok(false) => Ok(axis_domain::models::cloud::AuthStatus::Failed(
                "Not authenticated".into(),
            )),
            Err(e) => Ok(axis_domain::models::cloud::AuthStatus::Failed(
                e.to_string(),
            )),
        }
    }
